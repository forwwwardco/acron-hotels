...
<section id="exploreSuites" class="explore-suites-section py-5 bg-lightest-blue position-relative <?= $esc_html($suiteClass) ?>" aria-labelledby="suitesSectionHeading">
    <div class="container py-4 position-relative z-1">
        <h2 id="suitesSectionHeading" class="text-center fw-bold text-blue-grey mb-5 pb-lg-3 reveal section-heading"><?= $esc_html($heading) ?></h2>
        ...
        ?>
            ...
            <article class="row align-items-center gy-4 gy-lg-5 mb-5 mb-lg-5 pb-3 suite-row reveal">
                <div class="col-lg-6 <?= $textOrder ?>">
                    <h3 class="fw-bold text-blue-grey mb-3 fs-3"><?= $esc_html($suite['name']) ?></h3>
                    <p class="text-blue-grey opacity-75 mb-4 small fw-medium lh-lg <?= $isAlt ? '' : 'pe-lg-4' ?>"><?= nl2br($esc_html($suite['description'])) ?></p>
                    <div class="row gy-3 mb-4 suite-amenities">
                        <?php foreach ($suite['amenities'] as $amenity): ?>
                            <div class="col-6 d-flex align-items-center gap-2">
                                ...
                                <span class="fw-bold text-blue-grey opacity-75 small"><?= $esc_html($amenity['text']) ?></span>
                            </div>
                        <?php endforeach; ?>
                    </div>
                    <div class="d-flex flex-wrap gap-3 mt-2">
                        <button type="button" class="btn <?= $esc_html($btnClass) ?> fw-bold px-4 py-2 small trigger-book-engine">BOOK NOW</button>
                        <a href="<?= $esc_html($suite['learnLink']) ?>" class="btn btn-outline-suite fw-bold px-4 py-2 small" aria-label="Learn more about the <?= $esc_html($suite['name']) ?>">LEARN MORE</a>
                    </div>
                </div>
                        ...
                        <div id="<?= $carouselId ?>" class="carousel slide suite-carousel position-relative" data-bs-ride="carousel" data-bs-interval="5000" data-bs-pause="hover" aria-label="<?= $esc_html($suite['name']) ?> Image Gallery">
                            ...
                            <div class="carousel-inner h-100 rounded-4 shadow-sm">
                                <?php foreach ($suite['images'] as $imgIndex => $imgSrc): ?>
                                    <div class="carousel-item <?= $imgIndex === 0 ? 'active' : '' ?> h-100"><img src="<?= $esc_html($imgSrc) ?>" class="d-block w-100 h-100 object-fit-cover placeholder-img" alt="View <?= $imgIndex + 1 ?> of <?= $esc_html($suite['name']) ?>" loading="lazy"></div>
                                <?php endforeach; ?>
                            </div>
    ...
    <div class="wave-divider-bottom" aria-hidden="true"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1440 100" preserveAspectRatio="none"><path fill="var(--light-blue)" d="M0,40 C480,120 960,-40 1440,40 L1440,100 L0,100 Z"></path></svg></div>
</section>