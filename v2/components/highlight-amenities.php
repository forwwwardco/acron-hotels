...
<section id="highlightAmenities" class="highlight-amenities-section py-5 position-relative" aria-labelledby="highlightsHeading">
    <div class="container py-4 position-relative z-1">
        <h2 id="highlightsHeading" class="text-center fw-bold text-blue-grey mb-5 reveal section-heading"><?= $esc_html($heading) ?></h2>
        <div class="row gy-5 justify-content-center">
            <?php foreach ($highlights as $highlight): ?>
                <article class="col-12 col-md-6
                    <?php if ($pageResort == 'beachwalk' || $pageResort == 'regina'): ?> col-lg-4 <?php endif; ?>
                    <?php if ($pageResort == 'waterfront'): ?> col-lg-3 <?php endif; ?>
                        highlight-item reveal">
                    <img src="<?= $esc_html($highlight['image']) ?>" class="highlight-img placeholder-img mb-4 shadow-sm" alt="<?= $esc_html($highlight['title']) ?>" loading="lazy">
                    <h3 class="fw-bold text-blue-grey mb-3 fs-3"><?= $esc_html($highlight['title']) ?></h3>
                    <p class="text-blue-grey mb-0 small lh-lg opacity-85 fw-medium pe-lg-3"><?= nl2br($esc_html($highlight['description'])) ?></p>
                </article>
            <?php endforeach; ?>
        </div>
    </div>
