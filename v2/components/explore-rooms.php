            ...
            <article class="row align-items-center gy-4 gy-lg-5 mb-5 mb-lg-5 pb-3 room-row reveal position-relative">
                ...
                <div class="col-lg-6 <?= $textOrder ?>">
                    ...
                    <p class="text-blue-grey opacity-75 mb-4 small fw-medium lh-lg <?= $isAlt ? '' : 'pe-lg-4' ?>">
                        <?= nl2br($esc_html($room['description'])) ?>
                    </p>
                    ...
                    <div class="d-flex flex-wrap gap-3 mt-2">
                        <button type="button" class="btn <?= $esc_html($resortBtnClass) ?> fw-bold px-4 py-2 small trigger-book-engine">
                            BOOK NOW
                        </button>
                        <a href="<?= $esc_html($room['learnLink']) ?>" class="btn btn-outline-room fw-bold px-4 py-2 small" aria-label="Learn more about the <?= $esc_html($room['name']) ?>">
                            LEARN MORE
                        </a>
                    </div>
                </div>
                            ...
                            <div class="carousel-inner h-100 rounded-4 shadow-sm">
                                <?php foreach ($room['images'] as $imgIndex => $imgSrc): ?>
                                    <div class="carousel-item <?= $imgIndex === 0 ? 'active' : '' ?> h-100">
                                        <img src="<?= $esc_html($imgSrc) ?>" class="d-block w-100 h-100 object-fit-cover placeholder-img" alt="View <?= $imgIndex + 1 ?> of <?= $esc_html($room['name']) ?>" loading="lazy">
                                    </div>
                                <?php endforeach; ?>
                            </div>
